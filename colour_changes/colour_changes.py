...
def Load_Saved_Colour_Changes(default_theme, default_appearance):
    with open('user_details.py', 'r') as file:
        user_details = file.read()

    try:
        saved_theme = re.search(r"Saved_Theme = '([^']*)'", user_details).group(1)
        saved_appearance = re.search(r"Saved_Appearance = '([^']*)'", user_details).group(1)
